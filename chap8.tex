...
\newline
\newline
Delete the \texttt{filter-branch} backup using \indexgit{git update-ref <refname> -d}.  (See the callout on \emph{More backups})
\newline
\newline
...
We actually have at least three methods we can use to do this.
\begin{enumerate}
...
Now the first of these is trivial and should require no explanation at all.
...
The second method you should be familiar enough with pervious material to be able to perform right now.
...
What we have asked Git to do is to pull only the branch \textbf{tester_split} from the remote we called \textbf{source} and place it into \textbf{master} locally.
