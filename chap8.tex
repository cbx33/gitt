...
\index{bundling!creating}\index{bundling!cloning from}The \texttt{bundle} utility allows us to export a set of revisions and almost archive them to a file.
...
Git offers a tool called \indexgit{submodule}.  This tool allows you to link a remote repository into a branch and store it under a subdirectory of the project.
...
\item\texttt{git am <filename>} - Apply a specific patch containing a 
...
%john@satsuki:~/coderepo$ git bisect good v1.0a
%john@satsuki:~/coderepo$ git bisect bad master
%git bisect start master v1.0a

%git bisect run

%git filter-branch --index-filter 'git rm --cached --ignore-unmatch newfile1' HEAD
%git filter-branch --subdirectory-filter tester
%git fetch source +tester_split:master
%git branch -m tmp master
%git bundle create ../tester.bundle tester_split 
