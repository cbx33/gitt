...
\begin{itemize}
\item\index{sha1}\textbf{$<$SHA1$>$} - This is most common way to specify an object.
...
\item\index{refname}\textbf{$<$refname$>$} - We use this type of referencing a lot when checking out branches.  
...
\item\index{date}\textbf{$<$refname$>$@\{date\}} - Now we find a more interesting way of specifying references.
\end{itemize}
